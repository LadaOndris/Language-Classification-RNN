...
class Network:
    ...
    def __init__(self, n_samples, n_epochs, batch_size, n_classes, rnn_size, n_accuracy_testing_per_epoch,
                 saveModelAfterEachEpoch=True, savePath="/saved_models", measureEpochTime=True):
    ...
    def train_neural_network(self, x, y, learningRate, test_x, test_y, train_x, train_y, train_set_file_path):
        ...
        with tf.Session() as sess:
            ...
            # saver.restore(sess,  tf.train.latest_checkpoint("saved_models/v3/1"))
                    ...
                    for line in f:
                        ...
                        if batch_x_length >= self.batch_size:
                            _, c = sess.run([optimizer, cost],
                                            feed_dict={x: batch_x, y: batch_y, learning_rate: learningRate})
                            epoch_loss += c
                            errors.append(c)
                            ...
                            batch_y_length = 0
                            batches_run += 1
                            print('Batch run:', batches_run, '/', self.total_batches, '| Epoch:', epoch + 1,
                                  '| Batch Loss:', c, )
...
network = Network(n_samples=9984, n_epochs=5, batch_size=64, n_classes=5, rnn_size=512, \
                  n_accuracy_testing_per_epoch=5, savePath="saved_models/v3/2")
